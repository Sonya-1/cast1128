...
void Cast128::setByte(Cast128::Block& bl, uint8_t pos, uint8_t byte) {
...
Cast128::Block Cast128::go(const Key key, const Block Block, bool isEncrypt) {
	...
	for (int i = 0; i < roundCount; ++i) {
		...
		uint Kmi = K[rIndex]; //32-разрядные подключ маскировки
		uint8 Kri = K[16 + rIndex] & 0x1F; // 32-разрядные перестановка подключа
...
Cast128::Block Cast128::invertSecond(Block b) {
	uint32_t mask = 0b0101010101010101;
	b.Msg[0] = b.Msg[0] ^ mask;
	b.Msg[1] = b.Msg[1] ^ mask;
	
	return b;
}

Cast128::Block Cast128::invertThird(Block b) {
	uint32_t mask = 0b0010010010010010;
	b.Msg[0] = b.Msg[0] ^ mask;
	b.Msg[1] = b.Msg[1] ^ mask;
...
std::vector<Cast128::Block> Cast128::readFile(const char* inputFileName) {
	BMP img(inputFileName);
	std::vector<char> res;
	for (int i = 0; i < img.data.size(); i++) {
		res.push_back((char)img.data[i]);
	}
	img.data.clear();
...
void Cast128::encryptFile(const char* inputFileName, const char* outFileName, Cast128::Key key) {
	BMP img(inputFileName);
	std::vector<char> res;
	for (int i = 0; i < img.data.size(); i++) {
		res.push_back((char)img.data[i]);
...
void Cast128::decryptFile(const char* inputFileName, const char* outFileName, Cast128::Key key) {
	...
	writeLastBlock(img.data, encrypredBlock);
	img.write(outFileName);
}

void Cast128::writeLastBlock(std::vector<uint8_t>& out, Cast128::Block v) {
	char res[8];
	for (int i = 0; i < 8; ++i) {
...
void Cast128::readKey(const std::string str, Cast128::Key* key) {
	std::vector<char> res;
